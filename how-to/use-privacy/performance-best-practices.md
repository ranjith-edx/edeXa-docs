...
Private transactions use the same facilities as public ones. General edeXa performance tunings apply. Specific approaches are out of scope of this document, except for the following, which strongly impacts performance:

### Use fast, local, solid state storage

Running EVM transactions creates a lot of random reads that are executed sequentially. The edeXa data folder for high throughput nodes should be located on the fastest possible storage media.
...
### Co-locate edeXa and Tessera

edeXa has to talk to its local Tessera node frequently while handling a block. While we do not recommend running them on the same node, minimizing the latency between edeXa and Tessera will improve block processing times. edeXa and Tessera should not be hosted in geographically distributed locations.
...
For performance and reliability it is advantageous to statefully manage nonces on the client side instead of querying them for every transaction. If custom code for this is not an option, [Orchestrate](https://consensys.net/codefi/orchestrate/) can be used.
