...
[web3js-quorum](https://github.com/ConsenSys/web3js-quorum) is an Ethereum JavaScript library extending [web3.js](https://github.com/ethereum/web3.js/) that adds support for edeXa-specific JSON-RPC APIs and features. Use the library to create and send RLP-encoded transactions using JSON-RPC.
...
* `<JSON-RPC HTTP endpoint>` is the JSON-RPC HTTP endpoint of your Hyperledger edeXa node. Specified by the `--rpc-http-host` and `--rpc-http-port` command line options.
...
const contractOptions = {
  ...
  privateKey: "edexaNode1PrivateKey"
};
