...
edeXa implements the QBFT proof of authority (PoA) consensus protocol. QBFT is the recommended enterprise-grade consensus protocol for private networks.
...
The genesis file differs depending on the [validator management method](broken-reference) you intend to use.
...
* [`extraData`](broken-reference) - RLP encoded [extra data](broken-reference).
...
* If using:
  * [Block header validator selection](broken-reference), a list of validator addresses.
  * [Contract validator selection](broken-reference), no validators.
...
````
```bash
edexa rlp encode --from=toEncode.json --type=QBFT_EXTRA_DATA
```
````
...
Additionally, [`extraData`](broken-reference) is limited to the 32 bytes of vanity data after The Merge.

## Add and remove validators

QBFT provides two methods to manage validators:

* [Block header validator selection](broken-reference) - Existing validators propose and vote to add or remove validators using the QBFT JSON-RPC API methods.
* [Contract validator selection](broken-reference) - Use a smart contract to specify the validators used to propose and validate blocks.

You can use [transitions](broken-reference) to swap between block header validator selection and contract validator selection in an existing network.

For block header validator selection, initial validators are configured in the genesis file's [`extraData`](broken-reference) property, whereas the initial validators when using the contract validator selection method are configured in the genesis file's `storage` section.
...
The `transitions` genesis configuration item allows you to specify a future block number at which to change QBFT network configuration in an existing network. For example, you can update the [block time](broken-reference), [block reward](broken-reference), [validator management method](broken-reference), or [mining beneficiary](broken-reference).
