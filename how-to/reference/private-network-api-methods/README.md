...
* `fromBlockNumber`: _string_ - integer representing a block number or the string tag `earliest`, as described in Block Parameter
* `toBlockNumber`: _string_ - integer representing a block number or one of the string tags `latest` or `pending`, as described in Block Parameter
...
`result`: _string_ - 32-byte transaction hash of the privacy marker transaction
...
`enodes`: _array_ of _strings_ - list of enode URLs
...
`result`: _array_ of _strings_ - enode URLs of nodes in the nodes allowlist
...
* `privacyGroupId`: _string_ - 32-byte privacy Group ID
...
outside of edeXa, use [`priv_distributeRawTransaction`](../../how-to/send-transactions/private-transactions.md#priv_distributerawtransaction)
...
`result`: _array_ of _objects_ - privacy group objects containing only the specified members; privacy groups are EEA-compliant or edeXa-extended with types:

* `LEGACY` for EEA-compliant groups.
* `PANTHEON` for edeXa-extended groups.
...
edeXa instead of using `priv_getEeaTransactionCount`.
```
...
Filters for private contracts can only be created by [`priv_newFilter`](broken-reference) so unlike `eth_getFilterChanges`, `priv_getFilterChanges` always returns an array of log objects or an empty list.
...
`result`: _array_ of _objects_ - list of log objects, or an empty list if nothing has changed since the last poll
...
`transaction`: _string_ - transaction hash returned by [`eea_sendRawTransaction`](broken-reference) or [`eea_sendTransaction`](https://docs.ethsigner.consensys.net/en/latest/Using-EthSigner/Using-EthSigner/#eea\_sendtransaction).

#### Returns

`result`: _object_ - private transaction object, or `null` if not a participant in the private transaction
...
If sending more than one transaction to be mined in the same block (that is, you are not
waiting for the transaction receipt), you must calculate the private transaction nonce outside
edeXa instead of using `priv_getTransactionCount`.
```
...
`result`: _object_ - private Transaction receipt object, or `null` if no receipt found
...
Creates a log filter for a private contract. To poll for logs associated with the created filter, use [`priv_getFilterChanges`](broken-reference). To get all logs associated with the filter, use [`priv_getFilterLogs`](broken-reference).
...
Filters time out when not requested by [`priv_getFilterChanges`](broken-reference) or [`priv_getFilterLogs`](broken-reference) for 10 minutes.
