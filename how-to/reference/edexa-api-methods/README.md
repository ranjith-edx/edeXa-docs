...
`enode`: _string_ - enode URL of peer to add
...
* `fromBlock`: _string_ - integer representing a block number or one of the string tags `latest`, `earliest`, or `pending`, as described in Block Parameter
* `toBlock`: _string_ - integer representing a block number or one of the string tags `latest`, `earliest`, or `pending`, as described in Block Parameter
...
* `id`: _string_ - node public key (excluding the `0x` prefix, the node public key is the ID in the enode URL `enode://<id ex 0x>@<host>:<port>`.)
...
`enode`: _string_ - enode URL of peer to remove
...
Sends a list of signed transactions. This is used to quickly load a network with a lot of transactions. This does the same thing as calling [`eth_sendRawTransaction`](broken-reference) multiple times.
...
`blockNumber`: _string_ - integer representing a block number or one of the string tags `latest`, `earliest`, or `pending`, as described in Block Parameter
...
Use [`debug_standardTraceBadBlockToFile`](broken-reference) to view the trace for an invalid block.
...
Use [`debug_standardTraceBlockToFile`](broken-reference) to view the trace for a valid block.
...
`result`: _array_ of _objects_ - list of trace objects
...
You can interact with contracts using [`eth_sendRawTransaction`](broken-reference) or `eth_call`.
...
The `eth_estimateGas` call does not send a transaction. You must call [`eth_sendRawTransaction`](broken-reference) to execute the transaction.
...
* `newestBlock`: _string_ - Integer representing the highest number block of the requested range or one of the string tags `latest`, `earliest`, or `pending`, as described in Block parameter.

#### Returns

`result`: _object_ - Fee history results object.
...
`result`: _object_ - block object, or `null` when there is no block
...
* `blockNumber`: _string_ - integer representing a block number or one of the string tags `latest`, `earliest`, `pending`, `finalized`, or `safe` as described in Block Parameter
...
`result`: _array_ of _objects_ - list of log objects
...
`result`: _object_ - miner data object
...
`result`: _object_ - transaction object, or `null` when there is no transaction
...
`result`: _object_ - transaction receipt object, or `null` when there is no receipt
...
Creates a filter to retrieve new block hashes. To poll for new blocks, use [`eth_getFilterChanges`](broken-reference).
...
Creates a log filter. To poll for logs associated with the created filter, use [`eth_getFilterChanges`](broken-reference). To get all logs associated with the filter, use [`eth_getFilterLogs`](broken-reference).
...
Creates a filter to retrieve new pending transactions hashes. To poll for new pending transactions, use [`eth_getFilterChanges`](broken-reference).
...
You can interact with contracts using `eth_sendRawTransaction` or [`eth_call`](broken-reference).
...
* `currentBlock`: _string_ - index of the latest block (also known as the best block) for the current node (this is the same index that [`eth_blockNumber`](broken-reference) returns.)
...
Filters time out when not requested by [`eth_getFilterChanges`](broken-reference) or [`eth_getFilterLogs`](broken-reference) for 10 minutes.
...
Starts the mining process. To start mining, you must first specify a miner coinbase using the `--miner-coinbase` command line option or using [`miner_setCoinbase`](broken-reference).
...
`result`: _string_ - enode URL of the node
...
The `TRACE` API is a more concise alternative to the [`DEBUG` API](broken-reference).
...
`result`: _array_ of _objects_ - list of calls to other contracts containing one object per call, in transaction execution order; if revert reason is enabled with `--revert-reason-enabled`, the returned list items include the revert reason.
...
* `call`: _object_ - transaction call object
* `blockNumber`: _string_ - integer representing a block number or one of the string tags `latest`, `earliest`, or `pending`, as described in Block Parameter
* `options`: _array_ of _strings_ - list of tracing options; tracing options are `trace`, `vmTrace`, and `stateDiff`. Specify any combination of the three options including none of them.

#### Returns

`result`: _array_ of _objects_ - list of calls to other contracts containing one object per call, in transaction execution order
...
`result`: _array_ of _objects_ - list of calls to other contracts containing one object per call, in the order called by the transaction
...
`result`: _array_ of _objects_ - list of transaction trace objects containing one object per transaction, in transaction execution order; if revert reason is enabled with `--revert-reason-enabled`, the `trace` list items in the returned transaction trace object include the revert reason.
...
`result`: _array_ of _objects_ - list of calls to other contracts containing one object per call, in the order called by the transaction; if revert reason is enabled with `--revert-reason-enabled`, the returned list items include the revert reason.
...
`result`: _array_ of _objects_ - list of objects with details of the pending transaction
...
* `maxSize`: _number_ - maximum number of transactions kept in the transaction pool; use the `--tx-pool-max-size` option to configure the maximum size.
